...
public class FacilityService implements ChildEventListener, ValueEventListener {
    private DatabaseReference mLanguagePreference;

    public FacilityService(){
            mLanguagePreference = FirebaseDatabase.getInstance().getReference("facility");
    ...
    public FacilityService(Query ref) {
        mQuery = ref;
...
//    }
    ...
    public String getFacilityNameByKey(String key) {
        if (getCount() > 0) {
            for (int i = 0; i < getCount(); i++) {
    ...
    public String getFacilityKeyByType(int type) {
        if (getCount() > 0) {
            for (int i = 0; i < getCount(); i++) {
                if (getItem(i).getValue(Facility.class).getType() == type) {
                    return getItem(i).getKey();
                }
            }
        }
    ...
    public int getFacilityTypeByKey(String key) {
        if (getCount() > 0) {
            for (int i = 0; i < getCount(); i++) {
                if (getItem(i).getKey().equalsIgnoreCase(key)){
                    return getItem(i).getValue(Facility.class).getType();
                }
            }
        }
    ...
    public Facility getFacilityByKey(String key) {
        if (getCount() > 0) {
            for (int i = 0; i < getCount(); i++) {
                if (getItem(i).getKey().equalsIgnoreCase(key)){
                    String  lang_prefix= Uts.getInstance().lang_prefix();
                    Facility facility= getItem(i).getValue(Facility.class);
                    facility.setName((String)getItem(i).child("name"+lang_prefix).getValue());
                    return facility;
                }
            }
        }
